...
/* Includes ---------------------------------------------------------------- */
#include <IanRamotar-project-1_inferencing.h>
    ...
    while(stop_inferencing == false) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            ei_printf("    %s: %.5f\n", result.classification[ix].label,
                                        result.classification[ix].value); //result.classification 
        }
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) { //0 is asc, 1 chair, 2 desc, 3 table
             if(result.classification[ix].value >= 0.7){
                 switch(ix) {
                   ...
                   case 2:
                      for(int i=2000 ; i>100 ; i--){ // loop for to increase the sound frequency
                        //tone(buzzer, i);
                        delay(10); //You can change the delay if you want to make it longer or shorter
